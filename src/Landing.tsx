    ...
    <section className="relative isolate overflow-hidden pt-[68px] lg:pt-[78px]">
      ...
      <div className="mx-auto grid min-h-[900px] w-full max-w-[1400px] items-center gap-10 px-4 py-14 sm:min-h-[780px] sm:px-6 lg:min-h-[760px] lg:grid-cols-[1.04fr_.96fr] lg:gap-14 lg:px-10 lg:py-20">
        <div className="animate-rise max-w-2xl pt-5 lg:pt-0">
          <div className="hero-badge mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[.08] px-3 py-2 text-[10px] font-black tracking-[.16em] text-white/80 backdrop-blur sm:text-xs"><span className="h-2 w-2 animate-pulse rounded-full bg-flame" /> SMART DINING, BEAUTIFULLY SIMPLE</div>
          <h1 className="hero-title max-w-[11ch] text-[clamp(2.5rem,9vw,5.75rem)] font-black leading-[.94] tracking-[-.065em]">Dine smart.<br /><span className="text-flame">Order easy.</span></h1>
          <p className="hero-copy mt-5 max-w-xl text-[clamp(.98rem,2vw,1.18rem)] leading-7 text-white/68 sm:mt-6">Scan a table, discover dishes you will love, and follow every order from kitchen to table without waiting.</p>
          <div className="hero-actions mt-7 flex flex-col gap-3 sm:flex-row"><ActionButton onClick={() => navigate("/signup")} className="w-full sm:w-auto">Create Restaurant <ArrowRight size={17} /></ActionButton><ActionButton secondary onClick={goToMenu} className="w-full sm:w-auto"><QrCode size={17} /> Explore Menu</ActionButton></div>
